...
class AddForeignKeysToPaymentSaleReturnsTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('payment_sale_returns', function (Blueprint $table) {
            $table->foreign(['sale_return_id'], 'factures_sale_return')->references(['id'])->on('sale_returns');
            $table->foreign(['user_id'], 'factures_sale_return_user_id')->references(['id'])->on('users');
        });
    }
